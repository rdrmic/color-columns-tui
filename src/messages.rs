...
impl Message {
    pub const fn new_fading(text: &'static str, rgb: [u8; 3], fade_percent_per_tick: u8) -> Self {
        Self { text, rgb, opacity_percent: 100, fade_percent_per_tick }
    }

    pub const fn new_permanent(text: &'static str, rgb: [u8; 3]) -> Self {
        Self { text, rgb, opacity_percent: 100, fade_percent_per_tick: 0 }
    }
